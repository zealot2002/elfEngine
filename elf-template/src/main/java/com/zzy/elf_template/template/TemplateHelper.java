...
import com.zzy.elf_template.template.engine.BannerTemplateRender;
import com.zzy.elf_template.template.engine.RecyclerViewTemplateRender;
import com.zzy.elf_template.template.engine.SimpleTemplateRender;
import com.zzy.elf_template.template.engine.TemplateRender3;
...
public class TemplateHelper {
    public static final String TAG = "TemplateHelper";
    ...
    public static final HashMap<Integer,Integer[]> recyclerViewTemplateMap = new HashMap<Integer,Integer[]>() {{
        put(2,new Integer[]{R.layout.elf_template2, R.layout.elf_template2_item, RecyclerViewTemplateRender.RECYCLER_HORIZONTAL});
        put(6,new Integer[]{R.layout.elf_template6, R.layout.elf_template6_item,RecyclerViewTemplateRender.RECYCLER_VERTICAL});
        put(11,new Integer[]{R.layout.elf_template11, R.layout.elf_template11_item,RecyclerViewTemplateRender.RECYCLER_VERTICAL});
        put(9,new Integer[]{R.layout.elf_template9, R.layout.elf_template9_item,RecyclerViewTemplateRender.RECYCLER_GRID_3});
    }};
    ...
    public static final HashMap<Integer,Object[]> specialTemplateMap = new HashMap<Integer,Object[]>() {{
        put(3, new Object[]{TemplateRender3.class.getName(), R.layout.elf_template3});
    }};

/**************************************************************************************************************/
    public static SparseArray<TemplateRender> getEngineList(Context context, Page page) throws Exception {
        SparseArray<TemplateRender> templateRenderList = new SparseArray<>();
        for(Section section:(List<Section>)page.getBody().getDataList()){
            ...
            if(templateRenderList.get(templateId)==null){
                TemplateRender templateRender = getEngin(context,templateId);
                templateRenderList.put(templateId,templateRender);
            }
        }
        return templateRenderList;
    }

    public static TemplateRender getEngin(Context context, int templateId) throws Exception {
        if(simpleTemplateMap.containsKey(templateId)){
            return new SimpleTemplateRender().newInstance(context,templateId,simpleTemplateMap.get(templateId));
        }else if(recyclerViewTemplateMap.containsKey(templateId)){
            Integer[] params = recyclerViewTemplateMap.get(templateId);
            return new RecyclerViewTemplateRender().newInstance(context,templateId,params[0],params[1],params[2]);
        }else if(bannerTemplateMap.containsKey(templateId)){
            return new BannerTemplateRender().newInstance(context,templateId,bannerTemplateMap.get(templateId));
            ...
            TemplateRender render = (TemplateRender) (aClass.getConstructor().newInstance());
